...
public class JobApplicationItem extends Item {
    ...
    protected void addParticlesAroundSelf(Entity entity) {
        if(entity.level().isClientSide) {
            for(int i = 0; i < 5; ++i) {
                double d0 = entity.level().getRandom().nextGaussian() * 0.02;
                double d1 = entity.level().getRandom().nextGaussian() * 0.02;
                double d2 = entity.level().getRandom().nextGaussian() * 0.02;
