    ...
    public @NotNull InteractionResult interactLivingEntity(ItemStack stack, Player player, LivingEntity target, InteractionHand hand) {
        ...
        if (target instanceof Villager villager) {
            ...
            if (villager.isAlive() && hasThisJobUp.test(villager) && !villager.hasCustomName())  {
                ...
                //IF CLIENTSIDE TRUE, VILLAGER REACQUIRES TRADE. BUT IF !CLIENTSIDE TRUE, THIS DOESNT OCCUR. TEST
                if (!villager.level().isClientSide)  { //IF ON SERVER
                    ...
                    //player.level().addParticle(ParticleTypes.ELDER_GUARDIAN, player.getX(), player.getY(), player.getZ(), 0.0, 0.0, 0.0);
