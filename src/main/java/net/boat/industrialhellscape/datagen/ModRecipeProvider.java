...
//        slabRecipe(ModBlocks.RUSTY_RIVETED_VESSELPLATE_SLAB.get(), Ingredient.of(ModBlocks.RUSTY_RIVETED_VESSELPLATE_PANEL.get()),"rusty_riveted_vesselplate_slab_craft", pWriter);
    ...
    protected static void SingleBlockShapelessRecipe(Block outputBlock, Block inputBlock, Consumer<FinishedRecipe> pWriter) {
        ...
        ShapelessRecipeBuilder.shapeless(RecipeCategory.MISC, outputBlock, 1)
                .requires(inputBlock)
                .unlockedBy(getHasName(ModItems.INHELL_HAVEN_DEVICE.get()), has(ModItems.INHELL_HAVEN_DEVICE.get()))
                .save(pWriter, new ResourceLocation("industrialhellscape", "reversible_" + outputBlockName));

        ShapelessRecipeBuilder.shapeless(RecipeCategory.MISC, inputBlock, 1)
                .requires(outputBlock)
                .unlockedBy(getHasName(ModItems.INHELL_HAVEN_DEVICE.get()), has(ModItems.INHELL_HAVEN_DEVICE.get()))
                .save(pWriter, new ResourceLocation("industrialhellscape", "reversible_" + inputBlockName));
    }
